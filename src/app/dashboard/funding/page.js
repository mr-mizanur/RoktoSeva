...
const FundingPage = () => {
  ...
  // আগের ডোনেশনগুলো দেখানোর জন্য
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/funds`)
      .then(res => res.json())
  ...
  return (
    <div className="min-h-screen bg-[#070a13] text-white p-8">
      <div className="max-w-md mx-auto p-8 bg-[#0c101f] border border-white/10 rounded-3xl shadow-2xl text-center">
        ...
        <form action={`${process.env.NEXT_PUBLIC_SERVER_URL}/api/checkout_sessions`} method="POST">
          <div className="mb-6">
      ...
      {/* এখানে আপনি আগের দেওয়া টেবিল কোডটি যুক্ত করতে পারেন */}
    </div>
  );
};
